...
DF_PATTERN_OW = r"(\d+)$"
def load_data_ow(root_path, file_pattern=DF_PATTERN_OW):
    samples = {}
    pths = []
    for rt,drs,fnames in os.walk(root_path):
        pths.extend([os.path.join(rt,fname) for fname in fnames])
...
def main():
    ...
    parser.add_argument("-ld1", required=True)
    parser.add_argument("-ld2", required=True)
    ...
    print("+ loading data")
    samples_ow = load_data_ow(args.ld1)
    samples_cw = load_data(args.ld2)
    classes = list(samples_cw.keys())
    sample_leave_out = 0
    
    print("+ generating samples")
    X, p = [], []
    j = 0
    with tqdm(total=300*95) as pbar:
        while len(X) < 300*95:

            tabs = []

            # main sample
            sample = samples_ow[-1][j] #[np.random.randint(0, len(samples[cur_class]))]
            tabs.append(sample)
            j += 1
            ...
            for i in range(1):
                ...
                sample = samples_cw[tab_idx][np.random.randint(0, len(samples_cw[tab_idx]))]
                tabs.append(sample)
